...
import { jwtDecode } from 'jwt-decode' // <-- IMPORT THƯ VIỆN MỚI
    ...
    actions: {
        async login(credentials) {
            try {
                ...
                const { token } = response.data // API chỉ trả về token và username
                ...
                this.user = userData
                localStorage.setItem('user', JSON.stringify(userData)) // (Lưu ý: Chỉ lưu info, không lưu token trong user object)

                // 4. Điều hướng đến Dashboard
                // (Sử dụng 'replace' để người dùng không thể back lại trang login)
                router.replace('/')
        ...
        // (Action register sẽ được thêm sau)
    }
})