    ...
    <el-card class="box-card profile-card" v-if="authStore.user">
      <div class="profile-info">
        ...
        <div class="info-item">
          ...
          <span class="info-value">{{ user.data.email }}</span>
        </div>
        <div class="info-item">
        ...
        <div class="info-item">
          ...
          <span class="info-value">
            <el-tag v-for="role in authStore.user.roles" :key="role.id" class="role-tag">
              {{ role.name }}
            </el-tag>
          </span>
        </div>
...
onMounted(() => {
  // If user data is not available in the store, try to fetch it.
  // This might happen if the page is refreshed and the store state is lost,
  // but the token is still valid.
  if (!authStore.user && authStore.token) {
    // Assuming user ID can be derived from the token or is stored elsewhere
    // For now, we'll rely on the authStore.init() which should have run.
    // If authStore.user is null here, it implies a deeper issue or not logged in.
    // The v-if="authStore.user" handles the display.
    // No explicit fetch needed here as authStore.init() handles initial load.
  }
})
</script>
