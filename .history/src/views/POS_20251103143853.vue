...
const activeTab = ref('tables'); // Mặc định mở Sơ đồ Bàn

// Lấy state từ posStore
const tables = computed(() => store.getters.tables);
const currentCart = computed(() => store.getters.currentCart);
...
const handleTableClick = (table) => {
    ...
    if (currentCart.value.length > 0 && table.status === 'AVAILABLE') {
        if (confirm(`Gán đơn hàng hiện tại cho ${table.name}?`)) {
            store.dispatch('assignCartToTable', table.id);
        }
        return;
    }
    ...
    if (currentCart.value.length === 0 && table.status === 'PENDING') {
        ...
        if (table.pendingOrderId) {
            store.dispatch('selectOrder', table.pendingOrderId);
        } else {
            alert('Lỗi: Không tìm thấy đơn hàng cho bàn này.');
        }
        return;
    }
    ...
    if (currentCart.value.length === 0 && table.status === 'AVAILABLE') {
        // Chỉ cần báo cho nhân viên chọn món (không cần làm gì)
...
.pos-view {
    ...
    /* Chiều cao viewport trừ đi Navbar */
    overflow: hidden;
    /* Không cho phép cuộn toàn trang */
}
...
.cart-container {
    ...
    max-width: 450px;
    height: 100%;
    <template><div class_name="pos-view"><div class="pos-container"><div class="left-column"><div class="tabs"><button :class="{ active: activeTab === 'tables' }"
    @click="activeTab = 'tables'"
    >Sơ đồ Bàn </button><button :class="{ active: activeTab === 'orders' }"
    @click="activeTab = 'orders'"
    >Đơn hàng (Lịch sử) </button></div><div class="tab-content"><div v-if="activeTab === 'tables'" class="table-map"><CafeTable v-for="table in tables"
    :key="table.id"
    :table="table"
    @table-click="handleTableClick"

    /></div><div v-if="activeTab === 'orders'" class="order-history"><OrderHistoryList /></div></div></div><div class="right-column"><ActiveOrderDetails v-if="selectedOrder" /><div v-else class="default-view"><div class="menu-container"><ProductMenu /></div><div class="cart-container"><TemporaryCart /></div></div></div></div></div></template><script setup>import {
        ref,
        computed,
        onMounted
    }
    ...
    const handleTableClick=(table)=> {
        ...
        // 1. Nếu có hàng trong giỏ tạm VÀ bàn đang trống -> GÁN ĐƠN
        if (currentCart.value.length > 0 && table.status==='AVAILABLE') {
            if (confirm(`Gán đơn hàng hiện tại cho $ {
                        table.name
                    }
