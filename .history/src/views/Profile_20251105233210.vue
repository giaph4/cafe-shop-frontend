<template>
  <div class="app-page-container">
    ...
    <el-card class="box-card profile-card" v-if="userđ.user">
      <div class="profile-info">
        <div class="info-item">
          <span class="info-label">Tên đăng nhập:</span>
          <span class="info-value">{{ userđ.user.username }}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Họ và tên:</span>
          <span class="info-value">{{ userđ.user.fullName }}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Email:</span>
          <span class="info-value">{{ userđ.user.email }}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Số điện thoại:</span>
          <span class="info-value">{{ userđ.user.phone }}</span>
        </div>

        <div class="info-item">
          <span class="info-label">Trạng thái:</span>
          <span class="info-value">
            <el-tag :type="userđ.user.status === 'ACTIVE' ? 'success' : 'danger'">
              {{ userđ.user.status }}
            </el-tag>
          </span>
        </div>
        ...
        <div class="info-item">
          ...
          <span class="info-value">
            <el-tag v-for="role in userđ.roles" :key="role.id" class="role-tag">
              {{ role }}
            </el-tag>
          </span>
        </div>
...
<script setup>
import { ref } from 'vue'
import { userđ } from '@/api/userService'
import EditProfileModal from '@/components/EditProfileModal.vue'
import ChangePasswordModal from '@/components/ChangePasswordModal.vue'

const user = userđ()

console.log(userđ.user)
