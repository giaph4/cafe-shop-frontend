<template>
  <div class="app-page-container">
    ...
    <el-card class="box-card profile-card" v-if="getUserById.user">
      <div class="profile-info">
        <div class="info-item">
          <span class="info-label">Tên đăng nhập:</span>
          <span class="info-value">{{ getUserById.user.username }}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Họ và tên:</span>
          <span class="info-value">{{ getUserById.user.fullName }}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Email:</span>
          <span class="info-value">{{ getUserById.user.email }}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Số điện thoại:</span>
          <span class="info-value">{{ getUserById.user.phone }}</span>
        </div>

        <div class="info-item">
          <span class="info-label">Trạng thái:</span>
          <span class="info-value">
            <el-tag :type="getUserById.user.status === 'ACTIVE' ? 'success' : 'danger'">
              {{ getUserById.user.status }}
            </el-tag>
          </span>
        </div>
        ...
        <div class="info-item">
          ...
          <span class="info-value">
            <el-tag v-for="role in getUserById.roles" :key="role.id" class="role-tag">
              {{ role }}
            </el-tag>
          </span>
        </div>
...
import { usegetUserById } from '@/stores/auth'
