            ...
            <el-row :gutter="20">
                <el-col :span="10">
                    <el-card class="box-card menu-card">
                        <template #header>
                            <el-input v-model="productSearch" placeholder="Tìm món..." :prefix-icon="Search"
                                clearable />
                        </template>
                                ...
                                <div class="item-list">
                                    ...
                                    <div v-else class="cart-item" v-for="item in posStore.orderItems" :key="item.id">
                                        <div class="cart-item-info">
                                            ...
                                            <el-input v-model="item.notes" placeholder="Ghi chú (ít đá, nhiều đường...)"
                                                size="small" @change="(newNote) => onNoteChange(item.id, newNote)"
                                                class="cart-item-notes" />
                                        </div>
                                        <div class="cart-item-actions">
                                            <el-input-number :model-value="item.quantity"
                                                @change="(newQty) => onQuantityChange(item.id, newQty)" :min="1"
                                                size="small" class="cart-item-qty" />
                                ...
                                <div class="payment-tab">
                                    <el-form-item label="Tìm khách hàng (theo SĐT hoặc Tên)">
                                        <el-select v-model="selectedCustomerId" filterable remote reserve-keyword
                                            ...
                                            <el-option v-for="item in customers" :key="item.id"
                                                :label="`${item.fullName} - ${item.phone}`" :value="item.id" />
                                        </el-select>
                                    </el-form-item>
                                    ...
                                    <el-form-item label="Mã giảm giá">
                                        <el-input v-model="voucherInput" placeholder="Nhập mã voucher"
                                            ...
                                            <template #append>
                                                ...
                                                <el-button v-else :icon="X" type="danger" plain
                                    ...
                                    <el-form-item label="Chọn phương thức thanh toán">
                                        <div class="payment-methods">
                                            <el-button size="large" @click="onPay('CASH')" :icon="DollarSign">Tiền mặt
                                                (CASH)</el-button>
                                            <el-button size="large" @click="onPay('TRANSFER')" :icon="Landmark">Chuyển
                                                khoản
                                                (TRANSFER)</el-button>
                                            <el-button size="large" @click="onPay('CARD')" :icon="CreditCard">Thẻ
                                                (CARD)</el-button>
                                        </div>
                                    </el-form-item>
                                </div>
...
import { searchCustomersSimple } from '@/api/customerService'
...
onMounted(async () => {
    ...
    // Lọc các sản phẩm đang bán (frontend filter)
...
const onQuantityChange = (orderDetailId, newQuantity) => {
    posStore.updateItem(orderDetailId, { quantity: newQuantity, notes: null }) // notes: null để API không ghi đè
}

// Thay đổi ghi chú
let noteTimer = null
const onNoteChange = (orderDetailId, newNote) => {
    // Dùng debounce để không gọi API mỗi khi gõ
    clearTimeout(noteTimer)
    noteTimer = setTimeout(() => {
        posStore.updateItem(orderDetailId, { quantity: 0, notes: newNote }) // quantity: 0 để API không ghi đè
    }, 700)
}
...
const onPay = async (paymentMethod) => {
    ...
    if (success) {
        // Đã đóng modal trong store
    }
}

// Đồng bộ voucher input (nếu voucher được áp dụng)
