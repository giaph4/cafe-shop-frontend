<template>
    <div class="active-order-details" v-if="order">
            ...
            <div v-else v-for="item in order.orderDetails" :key="item.id" class="order-item">
                ...
                <div class="item-actions">
                    ...
                    <input type="number" :value="item.quantity"
                        @change="e => updateQuantity(item.id, parseInt(e.target.value))" class="item-quantity" />
                    <button @click="updateQuantity(item.id, item.quantity + 1)">+</button>
        ...
        <div class="order-footer">
            ...
            <button @click="handlePayment" class="btn-payment" :disabled="isLoading">
...
const order = computed(() => store.getters['pos/selectedOrder']);
const total = computed(() => store.getters['pos/selectedOrderTotal']);
const isLoading = computed(() => store.getters['pos/isLoading']);

// Đóng chi tiết
const closeDetails = () => {
    store.dispatch('pos/clearSelectedOrder');
};

const updateQuantity = (detailId, quantity) => {
    if (quantity < 0) return; // Không cho phép số âm

    if (quantity === 0) {
        if (confirm('Bạn có chắc muốn xóa món này?')) {
            store.dispatch('pos/removeOrderItem', detailId);
        }
    } else {
        store.dispatch('pos/updateOrderItemQuantity', { detailId, quantity });
    }
};
...
const handlePayment = () => {
    if (confirm('Xác nhận thanh toán cho đơn hàng này?')) {
        ...
        store.dispatch('pos/processPayment', paymentData);
    }
};
</script>
