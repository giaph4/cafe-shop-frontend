...
const total = computed(() => store.getters.currentCartTotal);
const isLoading = computed(() => store.getters.isLoading);

const updateQuantity = (productId, quantity) => {
    if (quantity > 0) {
        store.commit('UPDATE_CART_ITEM_QUANTITY', { productId, quantity });
    } else {
        store.commit('REMOVE_ITEM_FROM_CART', productId);
    }
};
...
const handleTakeaway = () => {
    // Action này sẽ tạo đơn hàng (status PENDING, tableId = null)
    // và tự động gán nó vào selectedOrder để sẵn sàng thanh toán
    store.dispatch('createTakeawayOrder');
};
</script>
