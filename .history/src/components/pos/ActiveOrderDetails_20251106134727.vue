<template>
    <div class="active-order-details" v-if="order">
        ...
        <div class="order-status-bar">
            ...
            <el-tag :type="order.status === 'PENDING' ? 'warning' : 'success'" effect="light">      {{ order.status }}
            </el-tag>
        ...
        <div class="order-footer">
            ...
            <div class="action-buttons">
                <el-button type="danger" @click="handleCancelOrder" :disabled="isLoading || order.status !== 'PENDING'"
...
const {
    ...
    cancelCurrentOrder // THÊM: Action mới, (sẽ gọi cancelOrder API)
} = posStore;
...
const updateQuantity = (detailId, quantity) => {
    // Action này (trong posStore) cần gọi updateItemInOrder hoặc removeItemFromOrder
...
const handlePayment = () => {
    ElMessageBox.confirm(
        ...
        const paymentData = {
            ...
            // (Nếu cần thêm phương thức thanh toán, bạn thêm ở đây)
            // paymentMethod: "CASH" 
        };
...
const handleCancelOrder = () => {
    ElMessageBox.confirm(
        ...
        // Action này (trong posStore) cần gọi API cancelOrder(order.id)
