<template>
    <el-card shadow="hover" class="cafe-table" :class="[tableClass, { 'is-selected': isSelected }]"
        @click="$emit('table-click', table)">
        <div class="table-content">
            <div class="table-name">{{ table.name }}</div>
            <div class="table-status">{{ statusText }}</div>
            <div v-if="table.status === 'PENDING' || table.status === 'OCCUPIED'" class="pending-order">
...
const props = defineProps({
    ...
    // THÊM: Prop để nhận biết bàn đang được CHỌN (cho luồng "chọn bàn trước")
    isSelected: {
        type: Boolean,
        default: false,
    }
...
const tableClass = computed(() => {
    ...
    switch (props.table.status) {
        ...
        case 'OCCUPIED':
            return 'occupied'; // Đỏ - Đang phục vụ
        case 'AVAILABLE':
...
/* THÊM: Xanh dương - Bàn ĐANG CHỌN */
.cafe-table.is-selected {
