<template>
    <div class="p-4 h-full flex flex-col" v-if="order">
      ...
      <div class="flex-grow overflow-y-auto pr-2">
        ...
        <div v-for="item in order.orderDetails" :key="item.id" class="mb-3 p-2 border rounded">
          ...
          <div v-else>
            ...
            <div class="flex items-center my-2">
              ...
              <input type="number" v-model.number="editForm.quantity" class="w-16 border rounded p-1 text-center" />
            </div>
            <input 
              type="text" 
        ...
        <button @click="showAddProductModal = true" class="w-full mt-2 p-2 border-2 border-dashed rounded text-blue-500 hover:bg-blue-50">
          + Thêm món
        </button>
      </div>
      ...
      <div class="mt-auto pt-4 border-t space-y-2">
        <div class="flex items-center" v-if="!order.voucher">
          <input v-model="voucherCodeInput" type="text" placeholder="Nhập mã voucher" class="flex-grow p-2 border rounded-l" />
        ...
        <div class="flex space-x-2 pt-2">
          <button @click="showCancelModal = true" class="w-1/3 bg-red-500 text-white p-2 rounded-lg font-semibold hover:bg-red-600">
            Hủy Đơn
          </button>
          <button @click="showPaymentModal = true" class="w-2/3 bg-green-500 text-white p-3 rounded-lg font-semibold hover:bg-green-600">
            Thanh Toán
          </button>
        </div>
      </div>
