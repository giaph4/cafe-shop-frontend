<template>
  <el-dialog v-model="dialogVisible" title="Đổi mật khẩu" width="500px" @close="handleClose">
...
// Giả sử file này export hàm changePassword(formData)
...
// SỬA LỖI: Đồng bộ validation với backend
const rules = reactive({
  ...
  newPassword: [
    ...
    // Sửa từ 8 -> 6 để khớp với ChangePasswordRequestDTO.java
...
const submitForm = async () => {
  ...
  formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        // SỬA LỖI: API không cần ID.
        // Backend (UserController) tự biết user là ai qua token.
        // File userService.js của bạn cần gọi:
        // apiClient.post('/api/v1/users/change-password', form)
