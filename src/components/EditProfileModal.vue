...
// Lấy thông tin cũ để hiển thị, đúng yêu cầu của bạn
onMounted(() => {
  if (authStore.user) {
...
const submitForm = async () => {
  ...
  formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      // SỬA LỖI: Xây dựng payload đầy đủ 5 trường
      // Backend (UserUpdateRequestDTO) yêu cầu 5 trường
      
      // 1. Lấy roleIds (mảng các số Long) từ authStore
      // Giả sử authStore.user.roles là [{id: 1, name: '...'}, {id: 2, name: '...'}]
      ...
      try {
        // Gọi API của Admin nhưng với ID của chính mình
        // API: PUT /api/v1/users/{id}
        // Hàm này trong userService.js của bạn có thể tên là
        // updateUserProfile, updateUser, updateUserById...
        await userService.updateUserProfile(authStore.user.id, updatePayload)
